    ...
    if scheme_symbolp(signature):
       ...
       # print("expre:", expressions)
        if isinstance(expressions.rest.first, Pair):
            second = scheme_eval(expressions.rest.first, env, True)
        else:
            second = expressions.rest.first
        env.define(signature, second)
        ...
        # bind variable value to a call do_lamda_form
        sthh = Pair(formals, body)
        value = do_lambda_form(sthh, env)
...
def do_and_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    # print("is scheme true:", is_scheme_true(expressions))
    # print("is false:", is_scheme_false(expressions))
    ...
    if expressions is not nil:
        ...
        if is_scheme_false(evaluated):
            result = evaluated
        # if false, set result to evalauted expr
        else:
            result = do_and_form(expressions.rest, env)
        return result
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    if expressions.rest is nil:
        # return evaluated expressions.first
        return scheme_eval(expressions.first, env, True)
    if expressions is not nil:
        evaluated = scheme_eval(expressions.first, env)
        # evaluate scheme eval ,first
        if is_scheme_true(evaluated):
            result = evaluated
        else:
            result = do_or_form(expressions.rest, env)
        return result
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        copy1 = bindings.first
        

        copy2 = copy1.rest.first
        
        names = Pair(copy1.first, names)
        
        vals = Pair(scheme_eval(copy2, env), vals)
